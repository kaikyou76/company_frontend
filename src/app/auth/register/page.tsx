...
import { registerUserAsync, checkUsernameAsync, selectLoading, selectError, selectRegisterSuccess, clearError, clearRegisterSuccess } from '@/app/store';
...
const RegisterPage = () => {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<ExtendedRegisterRequest>();
          ...
          <div className="rounded-md shadow-sm space-y-4">
            ...
            <div>
              ...
              {username && username.length >= 3 && isUsernameAvailable === false && (
                <p className="mt-1 text-sm text-red-600">用户名不可用或检查失败</p>
              )}
              {username && username.length >= 3 && isUsernameAvailable === true && (
                <p className="mt-1 text-sm text-green-600">用户名可用</p>
              )}
              {username && username.length >= 3 && isUsernameAvailable === null && (
                <p className="mt-1 text-sm text-gray-500">正在检查用户名...</p>
              )}
            </div>
            ...
            <div>
              ...
              <select
                id="locationType"
                {...register('locationType', { required: '工作地点类型是必填项' })}
                className="appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
              >
